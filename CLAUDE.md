...
- **Auth** — selezione in `build_mcp_auth` ([app/mcp/auth.py](app/mcp/auth.py), priorità **OAuth > bearer > none**):
  ...
  - entrambi off → `/mcp` **no-auth** (solo dev/Inspector — NON per prod).
