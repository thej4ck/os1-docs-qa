...
- `fetch(id)` → `{id,title,text,url,metadata}` — `query.mcp_fetch()` → `SearchIndex.get_document()` (match slash-tolerant come `/api/doc`). `id` = `source_file`. **`text`**: i marcatori `[Screenshot: cap | /help-files/…]` sono riscritti in markdown con URL **assoluti pubblici** (`_absolutize_screenshots`, usa `base_url`; logo saltati via `_is_logo`) → link immagine cliccabili dai client MCP (`/help-files` è pubblico). **Arricchimento VLM (build 101)**: se `image_descriptions` è popolata, l'alt-text markdown diventa la **caption VLM** (non l'heading), e le immagini di contenuto dei doc **help** (senza marcatori) vengono **iniettate** come `![caption](url)` via `SearchIndex.get_doc_images_for_fetch` (icone escluse) → l'AI esterna capisce gli screenshot senza vederli. **`metadata.images[]`**: array `{url assoluto, caption}` nell'ordine di apparizione nel `text` (per client che vogliono i riferimenti immagine strutturati, non solo il markdown inline).
- `view_images(urls)` → **content-block immagine NATIVI (base64)** — così Claude *vede* davvero gli screenshot, non solo i link (build 104). Il client (es. Claude) passa 1-2 URL scelti dalle caption di `metadata.images` di `fetch` (**la pertinenza la decide il chiamante**); il tool ritorna `fastmcp.utilities.types.Image(path=…)` → `ImageContent` webp base64. **Sicurezza**: whitelist via `SearchIndex.resolve_image(url)` (solo URL in `image_descriptions` con `vec IS NOT NULL` → niente path arbitrari/traversal, icone escluse); cap `MAX_VIEW_IMAGES=2`. NON tocca lo schema di `fetch` (tool separato → ChatGPT ignora, Claude usa). **WebP è formato vision ufficialmente supportato da Claude** (doc Anthropic: jpeg/png/gif/webp; media_type `image/webp`) → nessuna conversione necessaria. Le webp OS1 (<170KB, long-edge ≤~1200px) sono ben dentro i limiti (long-edge ≤1568px standard / ≤2576px high-res, ≤8000px, ≤5MB/immagine; downscaling automatico oltre). L'unica cosa da confermare dal vivo è che il connettore renderizzi il content-block (test di 30s post-deploy); conversione PNG via Pillow resta un fallback mai richiesto.
...
### Passata VLM immagini (`describe_images.py`) — lezioni operative (build 101-103)
- **Troncamento su report/tabelle → parse-fail SILENZIOSO.** Il VLM, su immagini di tabelle/report, tenta di trascrivere l'INTERA tabella in `ocr_text` → sfora `max_tokens` → `finish=length` → JSON incompleto → `_parse_json` ritorna `None` **senza eccezione** (per questo non compariva nei log). **Fix**: `ocr_text` vincolato nel prompt (~40 parole, "NON trascrivere righe di tabelle") + `response_format={"type":"json_object"}` + log separato `parse-fail` vs `VLM EXC`. Con questo: 4192/4192 ok. Se rivedi fallimenti, guarda `finish_reason`, non pensare subito al rate-limit.
- **È LATENZA, non rate-limit.** A concurrency 12 zero 429 (verificato); la lentezza è la latenza per-immagine di Sonnet vision (~7s). Si scala con `--concurrency`. Il rate-limit OpenRouter della chiave era di fatto illimitato; i limiti Anthropic a monte si alzerebbero solo via **BYOK**, ma non serve.
- **Salvataggio a blocchi (commit ogni 150).** L'incrementale `sha1` riprende SOLO tra run completati; un unico commit finale su un run da ~45 min è fragile (un'interruzione perde tutto). I commit per-blocco lo rendono crash-safe/resumibile.
- **Progress osservabile solo dalla spesa.** Mid-run il DB si aggiorna a fine blocco e lo stdout è bufferizzato → per stimare l'avanzamento leggi l'usage OpenRouter (`GET /api/v1/key`, ~$0.005/immagine).
- **Costo**: ~$26 one-time per ~4200 immagini (Sonnet 5 via OpenRouter).
- **`/help-files` è pubblico** (StaticFiles, no auth), `/api/doc` è auth-gated → il `fetch` MCP inietta URL `/help-files` (apribili da chiunque), MAI `/api/doc`. Un 401 su `/api/doc` in una segnalazione è l'URL sbagliato, non un bug.
- **`searchdata/search.db` ~56MB** dopo gli embedding immagini → warning GitHub >50MB (hard limit 100MB). Se cresce ancora: Git LFS o rigenerare in build invece di committare.
